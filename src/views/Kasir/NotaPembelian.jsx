...
// FIX 1 & 2: Perbaiki URL scheme RawBT dan hapus alert
...
const printDesktop = (printAreaRef) => {
  ...
  const ticketEl = printAreaRef.current.querySelector(".ticket")
  const ticketHTML = ticketEl ? ticketEl.outerHTML : ""
  ...
  doc.open()
  doc.write(`<!DOCTYPE html><html><head>${style}</head><body>${ticketHTML}</body></html>`)
  doc.close()
...
const NotaPembelian = ({ transactionData, onClose }) => {
  ...
  // FIX 3: Ganti tampilan HTML dengan kode yang diminta
  return (
        ...
        {transactionData?.items?.map((i, idx) => {
          ...
          return (
            <div key={idx} className="item">
              <div className="item-name">{namaBarang}</div>
              <div className="item-info">
                <div className="info-price">{rupiah(i.harga)}</div>
                <div className="info-qty">x{i.jumlah}</div>
                <div className="info-total">{rupiah(i.harga * i.jumlah)}</div>
              </div>
            </div>
          )
        })}
